...
int XAxiDma_Simple_MatProd(u16 DeviceId) {
	...
	for (col=0; col < N3; col++) {
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) TxBufferPtr, COLB_SIZE_IN_BYTES, XAXIDMA_DMA_TO_DEVICE);
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) (RxBufferPtr), COLC_SIZE_IN_BYTES, XAXIDMA_DEVICE_TO_DMA);
		...
		Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR) TxBufferPtr, MATA_SIZE_IN_BYTES, XAXIDMA_DMA_TO_DEVICE);
